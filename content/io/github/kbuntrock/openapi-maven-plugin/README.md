[io.github.kbuntrock:openapi-maven-plugin](https://central.sonatype.com/artifact/io.github.kbuntrock/openapi-maven-plugin/versions) RB check
=======

[![Reproducible Builds](https://reproducible-builds.org/images/logos/rb.svg) an independently-verifiable path from source to binary code](https://reproducible-builds.org/)

## Project: [io.github.kbuntrock:openapi-maven-plugin](https://central.sonatype.com/artifact/io.github.kbuntrock/openapi-maven-plugin/versions) [![Reproducible Builds](https://img.shields.io/endpoint?url=https://raw.githubusercontent.com/jvm-repo-rebuild/reproducible-central/master/content/io/github/kbuntrock/openapi-maven-plugin/badge.json)](https://github.com/jvm-repo-rebuild/reproducible-central/blob/master/content/io/github/kbuntrock/openapi-maven-plugin/README.md)

Source code: [https://github.com/kbuntrock/openapi-maven-plugin.git](https://github.com/kbuntrock/openapi-maven-plugin.git)

rebuilding **7 releases** of io.github.kbuntrock:openapi-maven-plugin:
- **7** releases were found successfully **fully reproducible** (100% reproducible artifacts :white_check_mark:),
- 0 had issues (some unreproducible artifacts :warning:, see eventual :mag: diffoscope and/or :memo: issue tracker links):

| version | [build spec](/BUILDSPEC.md) | [result](https://reproducible-builds.org/docs/jvm/): reproducible? | [stabilize](https://github.com/google/oss-rebuild/blob/main/cmd/stabilize/README.md) | size |
| -- | --------- | ------ | ------ | -- |
| [0.0.34](https://central.sonatype.com/artifact/io.github.kbuntrock/openapi-maven-plugin/0.0.34/pom) | [mvn jdk17](openapi-maven-plugin-0.0.34.buildspec) | [result](openapi-maven-plugin-0.0.34.buildinfo): [3 :white_check_mark: ](openapi-maven-plugin-0.0.34.buildcompare) | | 394K |
| [0.0.33](https://central.sonatype.com/artifact/io.github.kbuntrock/openapi-maven-plugin/0.0.33/pom) | [mvn jdk17](openapi-maven-plugin-0.0.33.buildspec) | [result](openapi-maven-plugin-0.0.33.buildinfo): [3 :white_check_mark: ](openapi-maven-plugin-0.0.33.buildcompare) | | 394K |
| [0.0.32](https://central.sonatype.com/artifact/io.github.kbuntrock/openapi-maven-plugin/0.0.32/pom) | [mvn jdk17](openapi-maven-plugin-0.0.32.buildspec) | [result](openapi-maven-plugin-0.0.32.buildinfo): [3 :white_check_mark: ](openapi-maven-plugin-0.0.32.buildcompare) | | 385K |
| [0.0.31](https://central.sonatype.com/artifact/io.github.kbuntrock/openapi-maven-plugin/0.0.31/pom) | [mvn jdk17](openapi-maven-plugin-0.0.31.buildspec) | [result](openapi-maven-plugin-0.0.31.buildinfo): [3 :white_check_mark: ](openapi-maven-plugin-0.0.31.buildcompare) | | 385K |
| [0.0.30](https://central.sonatype.com/artifact/io.github.kbuntrock/openapi-maven-plugin/0.0.30/pom) | [mvn jdk17](openapi-maven-plugin-0.0.30.buildspec) | [result](openapi-maven-plugin-0.0.30.buildinfo): [3 :white_check_mark: ](openapi-maven-plugin-0.0.30.buildcompare) | | 385K |
| [0.0.29](https://central.sonatype.com/artifact/io.github.kbuntrock/openapi-maven-plugin/0.0.29/pom) | [mvn jdk17](openapi-maven-plugin-0.0.29.buildspec) | [result](openapi-maven-plugin-0.0.29.buildinfo): [3 :white_check_mark: ](openapi-maven-plugin-0.0.29.buildcompare) | | 326K |
| [0.0.28](https://central.sonatype.com/artifact/io.github.kbuntrock/openapi-maven-plugin/0.0.28/pom) | [mvn jdk17](openapi-maven-plugin-0.0.28.buildspec) | [result](openapi-maven-plugin-0.0.28.buildinfo): [3 :white_check_mark: ](openapi-maven-plugin-0.0.28.buildcompare) | | 315K |

<i>(size is calculated without javadoc, that has been excluded from reproducibility checks)</i>
