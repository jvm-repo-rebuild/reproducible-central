...
displayOptional  "newlineGit"  "$newline"
[ -z "${newlineGit}" ] && newlineGit=$newline
checkNewline "$newlineGit"
...
"${SCRIPTDIR}/build_stabilize.sh" "$(dirname ${buildspec})/$(basename ${buildinfo} .buildinfo).buildcompare" buildcache/${artifactId}
