...
rebuildToolMvn() {
  ...
  local mvn_rebuild="${command} -V -e org.apache.maven.plugins:maven-artifact-plugin:3.5.3:compare org.apache.maven.plugins:maven-artifact-plugin:3.5.3:describe-build-output -Dbuildinfo.reproducible -Dcompare.fail=false"
  if [ -n "${referenceRepo}" ]
  then
