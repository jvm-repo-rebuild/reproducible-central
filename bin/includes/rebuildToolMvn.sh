...
rebuildToolMvn() {
  ...
  local mvn_rebuild="${command} -V -e org.apache.maven.plugins:maven-artifact-plugin:3.7.0:compare org.apache.maven.plugins:maven-artifact-plugin:3.7.0:describe-build-output -Dcompare.fail=false"
  case ${tool} in
    mvn-3.6.*|mvn-3.5.*)
