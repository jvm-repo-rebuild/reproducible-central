...
echo "- on $((countVersion - countVersionOk)) releases with issues :warning:, $countVersionStabilizedOk had all their unreproducible differences removed when stabilized :recycle:, $((countVersion - countVersionOk - countVersionStabilizedOk)) still had unreproducible differences when stabilized :rotating_light: or files are not supported by stabilize :no_entry_sign:" >> ${projectReadme}
echo >> ${projectReadme}
